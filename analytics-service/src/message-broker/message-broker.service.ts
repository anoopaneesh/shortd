...
export class MessageBrokerService {
    ...
    public async subscribe(subFn: EachMessageHandler) {
        this.consumer = this.kafka.consumer({
            groupId: this.configService.get('kafka.analytics_group') || "analytics_group"
        })
        await this.consumer.connect()
        await this.consumer.subscribe({ topic: this.configService.get('kafka.analytics_topic') || "analytics_topic", fromBeginning: true })
